...
const dataNetwork = JSON.parse(raw('../data/industry-space-no-overlap.json'));
const dataUMAP = JSON.parse(raw('../data/umap-raw.json'));
...
const createForceGraph = (rootEl, data) => {
  ...
  const allXValues = [];
  const allYValues = [];
  data.nodes.forEach(({initial_x, initial_y}) => {
    allXValues.push(initial_x);
    allYValues.push(initial_y);
  });
  ...
  dataUMAP.forEach(n => {
    ...
    if (!industry6Digit) {
      console.warn('undefined industry', n);
    } else {
      ...
      transformedNodes.push({
        ...n, radius, color,
        new_x: xScale(n.initial_x),
        new_y: yScale(n.initial_y),
      });
    }
  });
  ...
  function initGraph(tempData){
    ...
    function simulationUpdate() {
      ...
      tempData.nodes.forEach(function(d, i) {
        context.beginPath();
        context.arc(xScale(d.initial_x), yScale(d.initial_y), d.radius, 0, 2 * Math.PI, true);
      ...
      linkedEdges.forEach(function(d) {
        ...
        // context.beginPath();
        // context.moveTo(xScale(d.source.initial_x), yScale(d.source.initial_y));
        // context.lineTo(xScale(d.target.initial_x), yScale(d.target.initial_y));
      ...
      if (hoveredNode) {
        ...
        context.beginPath();
        context.arc(xScale(hoveredNode.initial_x), yScale(hoveredNode.initial_y), hoveredNode.radius, 0, 2 * Math.PI, true);
      ...
      if (highlightedNode) {
        context.beginPath();
        context.arc(xScale(highlightedNode.initial_x), yScale(highlightedNode.initial_y), highlightedNode.radius, 0, 2 * Math.PI, true);
